      ...
      createHolidayCharityProposal: async (
        ...
        try {
          ...
          const proposal: HolidayCharityProposal = {
            ...
            title: `Holiday Charity Selection - ${holidayId}`,
            author: 'system',
            status: 'active',
            ...
            noPercentage: 0,
            abstainPercentage: 0,
            description: `Select which charity will receive $${fundAmount.toLocaleString()} in funding for this holiday.`,
            createdAt: now,
            votingEndsAt,
            holidayId,
            availableCharities,
      ...
      submitVote: async (proposalId, vote, votingPower) => {
        ...
        try {
          ...
          if (proposal) {
            ...
            if (proposal.votingType === 'charity_selection') {
              // For charity selection, we don't update percentages the same way
              // This would be handled differently in the UI
              console.log(
                `Voted for charity: ${vote} on proposal: ${proposalId}`
              );
            } else {
              // Standard Yes/No/Abstain voting
              const totalVotes =
